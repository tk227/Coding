...
// variables to track the current and origin tower during dragging
let currentTower
let originTower

// element to display the move steps
...
async function moves(movements) {
  outputMoves.innerHTML = ""; // Clear previous moves
  for (let i = 0; i < movements.length; i++) {
    const element = movements[i];
    
    // Animate the disc movement
    moveTopDisc(element.origin, element.destiny);
    
    // Output the move as text
    const moveText = `Move disc from Tower ${element.origin + 1} to Tower ${element.destiny + 1}`;
    const moveElement = document.createElement('p');
    moveElement.textContent = moveText;
    outputMoves.appendChild(moveElement);
...
class Game {
  ...
  newGame = () => {
    // Event listener for the speed range input
